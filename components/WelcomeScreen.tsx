...
export function WelcomeScreen({ onNext, onLegal }: { onNext: () => void; onLegal?: (page: string) => void }) {
  const [trackWidth, setTrackWidth] = React.useState(0);
  const trackRef = React.useRef<HTMLDivElement>(null);
  const knobSize = 52;
  const padding = 12; // 6px padding on each side of the track
  ...
  const rightConstrain = trackWidth > 0 ? trackWidth - knobSize - padding : 240;
    ...
    <div className="flex-1 flex flex-col text-[#fff6ef] p-6 relative z-0 h-full">
      ...
      >
         ...
         <motion.div
           ...
           dragSnapToOrigin
           onDragEnd={(e, info) => {
             if (info.offset.x > rightConstrain * 0.8) onNext();
           }}
      ...
      <p className="text-[12px] text-center opacity-60 leading-[1.4] max-w-[280px] mx-auto mb-4 text-[#fffaf4]">
        Продолжая, вы принимаете <span onClick={() => onLegal?.('privacy')} className="underline underline-offset-2 decoration-white/30 cursor-pointer hover:text-white hover:decoration-white/60">политику обработки данных</span> и даёте согласие на <span onClick={() => onLegal?.('consent')} className="underline underline-offset-2 decoration-white/30 cursor-pointer hover:text-white hover:decoration-white/60">их обработку</span>
      </p>
    </div>
  );
}
